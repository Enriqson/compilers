...
class Regex:
    ...
    def is_operator(element):
        op_regex = re.compile(rf'^[{ACCEPTED_OPS}]$')
        result = op_regex.match(element)
        return result is not None

    @staticmethod
    def is_number(element):
        num_regex = re.compile(r'^[-+]?(?:\d*\.*\d+)$')
        result = num_regex.match(element)
        return result is not None
...
def tokenize_item(element):
    type = ""
    lexeme = ""
    if(Regex.is_number(element)):
        type = NUM_TYPE
        lexeme = element
    elif Regex.is_operator(element):
